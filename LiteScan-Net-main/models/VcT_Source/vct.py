...
import models
from .help_funcs import Transformer, TransformerDecoder, TwoLayerConv2d, TransformerCross
from .gcnlayers import *
from .kmeans import *
...
class Reliable_Transformer(ResNet):
    ...
    def __init__(self, input_nc, output_nc, with_pos, resnet_stages_num=5, token_trans=True,
        ...
        # [新增修复] 动态获取当前文件路径，加载 txt
        current_dir = os.path.dirname(os.path.abspath(__file__))
        path0 = os.path.join(current_dir, 'indice0.txt')
        path1 = os.path.join(current_dir, 'indice1.txt')

        # 加载并注册为 buffer (这样会自动随模型移动到 GPU)
        try:
            ind0 = torch.tensor(np.loadtxt(path0), dtype=torch.long)
            ind1 = torch.tensor(np.loadtxt(path1), dtype=torch.long)
            self.register_buffer('indice0', ind0)
            self.register_buffer('indice1', ind1)
